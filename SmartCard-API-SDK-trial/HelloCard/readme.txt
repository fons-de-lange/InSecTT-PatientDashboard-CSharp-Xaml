...
EMVcard     sample code for Euro-Visa-Master credit & debit cards, accessing static card data
...
HID_CARD    sample code for HID physical access control systems (PACS) cards
            -> PROX   sample code for HID PROX card access
            -> iCLASS sample code for HID iCLASS card access on OMNIKEY 5x21, 5x22, 5023 and 5x27 readers (available upon request)
            -> SEOS   sample code for HID SEOS card access on OMNIKEY 5023, and 5x27 readers (available upon request)
...
SIMcard     sample code for GSM SIM cards: reads phone records from card (requires correct user PIN and can lock your card if wrong PIN is entered)
...
Project examples:
- develop card personalization software/plugin for printer with built-in chip card encoder
